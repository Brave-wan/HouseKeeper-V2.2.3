...
public abstract class DPCalendar {
     ...
     * @return
     */
    public boolean isPost(int year, int month, Integer integer, List<Calendar> calendars) {
        Calendar c1 = Calendar.getInstance();
        c1.set(year, month - 1, integer);
        for (Calendar calendar : calendars) {
            if ((c1.get(Calendar.YEAR) == calendar.get(Calendar.YEAR)) &&
                    (c1.get(Calendar.MONTH) == (calendar.get(Calendar.MONTH))) &&
                    (c1.get(Calendar.DAY_OF_MONTH) == calendar.get(Calendar.DAY_OF_MONTH))) {
                return true;
            }
        }
        return false;
    ...
    public boolean isLeak(int year, int month, Integer integer, List<Calendar> calendars) {
        Calendar c1 = Calendar.getInstance();
        c1.set(year, month - 1, integer);
        for (Calendar calendar : calendars) {
            if ((c1.get(Calendar.YEAR) == calendar.get(Calendar.YEAR)) &&
