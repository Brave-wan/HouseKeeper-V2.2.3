...
public class MonthView extends View {
    ...
    private boolean isScroll = false; //是否允许滑动
    ...
    public MonthView(Context context, List<Calendar> postDate, List<Calendar> leakDate) {
        super(context);
        mCManager = DPCManager.getInstance(postDate,leakDate);
...
//                                }
    ...
    private void drawBG(Canvas canvas, Rect rect, DPInfo info) {
        ...
        } else if (info.isLeak){//漏检
            drawBGLeak(canvas, rect);
        } else if (info.isPost){//报事
            drawBGPost(canvas, rect);
        }else {
    ...
    private void drawBGPost(Canvas canvas, Rect rect) {
        mPaint.setColor(mTManager.colorPost());
        canvas.drawCircle(rect.centerX(), rect.centerY(), circleRadius / 3F, mPaint);
    }

    private void drawBGLeak(Canvas canvas, Rect rect) {
        mPaint.setColor(mTManager.colorLeak());
        canvas.drawCircle(rect.centerX(), rect.centerY(), circleRadius / 3F, mPaint);
    }
