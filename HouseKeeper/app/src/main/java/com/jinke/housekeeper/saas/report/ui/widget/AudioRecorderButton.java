...
import com.jinke.housekeeper.saas.report.util.AudioManager;
import com.jinke.housekeeper.utils.DialogManager;
...
public class AudioRecorderButton extends android.support.v7.widget.AppCompatButton {
    ...
    public AudioRecorderButton(Context context, AttributeSet attrs) {
        ...
        //录音文件存放地址
        String dir = Environment.getExternalStorageDirectory() + "/houseKeeper_voice";
    ...
    public void setStartRecorderCallBack(AudioStartRecorderCallBack listener) {
        startRecorderCallBack = listener;
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_UP://手指放开
                ...
                if (!isRecording || mTime < 2f) {//如果时间少于2s，则提示录音过短
    ...
    private void changeState(int state) {
        if (mCurrentState != state) {
            ...
            switch (state) {
                case STATE_NORMAL:
                    setBackgroundResource(R.drawable.voice_btn_unpress);
                    setText("按住说话");
                    break;

                case STATE_RECORDING:
                    setBackgroundResource(R.drawable.voice_btn_press);
                    setText("松开结束");
                    if (isRecording) {
