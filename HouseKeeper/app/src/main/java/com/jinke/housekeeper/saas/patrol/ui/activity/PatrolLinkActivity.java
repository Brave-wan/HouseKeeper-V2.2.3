...
public class PatrolLinkActivity extends BaseActivity {

    private int labelActivity;// 1 PatrolBeganActivity 2 PatrolAddActivity 3 PatrolReplaceActivity
    public final static int PatrolBeganActivity = 9211;
    public final static int PatrolAddActivity = 9212;
    public final static int PatrolReplaceActivity = 9213;
...
//    private String projectId;
    ...
    protected int getContentViewId() {
        return R.layout.activity_patrol_link;
    }

    @Override
    protected void initView() {
        setTitle(getString(R.string.activity_patrol_link));
    ...
    @OnClick({R.id.patrol_link})
    protected void buttonOnClock(View view) {
        switch (view.getId()) {
            case R.id.patrol_link:
                initBluetooth();
                break;
        }
    }
    ...
    private void initDate() {
        ...
        labelActivity = labelIntent.getIntExtra("label_activity", 0);
        if (0 == labelActivity)
            finish();
    }
    ...
    private DeviceManagerCallback deviceManagerCallback = new DeviceManagerCallback() {
                    ...
                    public void run() {
                        try {
                            ...
                            if ((bleNfcDevice.isConnection() == BleManager.STATE_CONNECTED)
                                ...
                                if (1 == labelActivity) {
                                    //员工签到
                                    startActivityForResult(new Intent(PatrolLinkActivity.this, PatrolBeganActivity.class), PatrolBeganActivity);
                                } else if (2 == labelActivity) {
                                    if (null !=  CommonlyUtils.getUserInfo(PatrolLinkActivity.this).getLeftOrgId()) {
                                        //新增点位
                                        Intent patrolAddActivityIntent = new Intent(PatrolLinkActivity.this, PatrolAddActivity.class);
                                        startActivityForResult(patrolAddActivityIntent, PatrolAddActivity);
                                    } else {
                                        ToastUtils.showShort("未获取到项目相关信息");
                                    }
                                } else if (3 == labelActivity) {
                                    //替换点位
                                    if (null !=  CommonlyUtils.getUserInfo(PatrolLinkActivity.this).getLeftOrgId()) {
                                        Intent patrolAddActivityIntent = new Intent(PatrolLinkActivity.this, PatrolReplaceActivity.class);
                                        startActivityForResult(patrolAddActivityIntent, PatrolReplaceActivity);
                                    } else {
                                        ToastUtils.showShort("未获取到项目相关信息");
                                    }
                                }
        ...
        public void onReceiveDisConnectDevice(boolean blnIsDisConnectDevice) {
            ...
            new AlertDialog.Builder(PatrolLinkActivity.this.getApplicationContext()).setTitle("提示")//设置对话框标题
                                ...
                                if (mNearestBle != null) {
                                    ...
                                    if (null == mNearestBle || "".equals(mNearestBle.getAddress())) {
                                        runOnUiThread(new Runnable() {
                                            @Override
                                            public void run() {
                                                new AlertDialog.Builder(PatrolLinkActivity.this).setTitle("提示")//设置对话框标题
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (requestCode) {
            case PatrolBeganActivity:
            case PatrolAddActivity:
            case PatrolReplaceActivity:
                finish();
                break;
            case 1313:
                switch (resultCode) {
