...
public final class DPCManager {
    ...
    private DPCManager(List<Calendar> postDate, List<Calendar> leakDate) {
        if (null != postDate) {
            this.postDate = postDate;
        } else {
            this.postDate = new ArrayList<>();
        }
        if (null != leakDate) {
            this.leakDate = leakDate;
        } else {
            this.leakDate = new ArrayList<>();
        }
        // 默认显示为中文日历
     ...
     * @return 月历管理器
     */
    public static DPCManager getInstance(List<Calendar> postDate, List<Calendar> leakDate) {
        if (null == sManager) {
            sManager = new DPCManager(postDate, leakDate);
        }else {
            sManager.setInfoDate(postDate, leakDate);
        }
        return sManager;
    }
        ...
        for (int i = 0; i < info.length; i++) {
            for (int j = 0; j < info[i].length; j++) {
                ...
                if (!TextUtils.isEmpty(tmp.strG)) {
                    ...
                    tmp.isPost =
                            c.isPost(year, month, Integer.valueOf(tmp.strG),postDate);
                    tmp.isLeak =
                            c.isLeak(year, month, Integer.valueOf(tmp.strG),leakDate);
                }
