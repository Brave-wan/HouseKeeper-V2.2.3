package com.jinke.housekeeper.saas.patrol.ui.widget;
...
public class ChartView extends View {
    //xy坐标轴颜色
    private int xylinecolor = 0x555555;
    //xy坐标轴宽度
    private int xylinewidth = dpToPx(1);
    //xy坐标轴文字颜色
    private int xytextcolor = 0x555555;
    //xy坐标轴文字大小
    private int xytextsize = spToPx(12);
    //折线图中折线的颜色
    private int linecolor = 0x78C7FE;
    //x轴各个坐标点水平间距
    ...
    //折线对应的数据
    private List<Map<String, Float>> value = new ArrayList<>();
    //点击的点对应的X轴的第几个点，默认1
...
//            //X轴文本最大高度
    ...
    private void drawBrokenPoint(Canvas canvas) {
        ...
        //绘制节点对应的原点
        int n = 0;
        for (Map<String, Float> map : value) {
            for (int i = 0; i < xValue.size(); i++) {
                ...
                switch (n) {
                    case 0:
                        x = xInit + interval * i;
                        y = yOri - yOri * (1 - 0.1f) * map.get(xValue.get(i)) / yValue.get(yValue.size() - 1);
                        //绘制选中的点
                        if (i == selectIndex - 1) {
                            linePaint.setStyle(Paint.Style.FILL);
                            linePaint.setColor(0xffd0f3f2);
                            canvas.drawCircle(x, y, dp7, linePaint);
                            linePaint.setColor(0xff78c7fe);
                            canvas.drawCircle(x, y, dp4, linePaint);
                            drawFloatTextBox(canvas, x, y - dp7, map.get(xValue.get(i)));
                        }
                        //绘制普通的节点
                        linePaint.setStyle(Paint.Style.FILL);
                    ...
                    case 1:
                        ...
                        if (i == selectIndex - 1) {
                            linePaint.setStyle(Paint.Style.FILL);
                            linePaint.setColor(0x88FFA964);
                            canvas.drawCircle(x, y, dp7, linePaint);
                            linePaint.setColor(0xffFFA964);
                            canvas.drawCircle(x, y, dp4, linePaint);
                            drawFloatTextBox(canvas, x, y - dp7, map.get(xValue.get(i)));
                        }
                        //绘制普通的节点
    ...
    private void drawBrokenLine(Canvas canvas) {
        ...
        for (Map<String, Float> map : value) {
            ...
            float y = yOri - yOri * (1 - 0.1f) * map.get(xValue.get(0)) / yValue.get(yValue.size() - 1);
            path.moveTo(x, y);
            path.moveTo(x, y + 20);
    ...
    private void drawXY(Canvas canvas) {
        ...
        for (int i = 0; i < xValue.size(); i++) {
            ...
            if (x >= xOri) {//只绘制从原点开始的区域
                ...
                //绘制X轴文本
                String text = "";
                String [] strings = xValue.get(i).split("");
    ...
    private void clickAction(MotionEvent event) {
        ...
        for (int i = 0; i < xValue.size(); i++) {
            ...
            for (Map<String, Float> map : value) {
                float x = xInit + interval * i;
                float y = yOri - yOri * (1 - 0.1f) * map.get(xValue.get(i)) / yValue.get(yValue.size() - 1);
