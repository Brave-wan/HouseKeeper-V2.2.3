...
import com.jinke.housekeeper.saas.report.presenter.InquireFilterFragmentPresenter;
import com.jinke.housekeeper.saas.report.ui.activity.fragmentworkbench.RectProcessActivity;
import com.jinke.housekeeper.saas.report.ui.adapter.AllReportAdapter;
...
public class InquireFilterFragment extends BaseFragmentV4<InquireFilterFragmentView, InquireFilterFragmentPresenter> implements
        View.OnClickListener,
        OnRefreshListener, BaseQuickAdapter.OnItemClickListener,OnLoadmoreListener,
        LoadingLayout.OnReloadListener, InquireFilterFragmentView {
    ...
    private AllReportAdapter allReportAdapter;//全部报事
    ...
    private void getAllReportList(int pageInfo) {
        ...
        if (info != null) {
            ...
            if (info.getSponsor() != null && info.getSponsor().toString().trim() != "") {
                map.put("superviseName", info.getSponsor());
            }
            if (info.getRectificationMan() != null && info.getRectificationMan().toString().trim() != "") {
                map.put("staffName", info.getRectificationMan());
            }
            ...
            if (info.getKeyPointId() != null && info.getKeyPointId().toString().trim() != "") {
                map.put("cruxId", info.getKeyPointId());
            }
        }
    ...
    @Override
    public InquireFilterFragmentPresenter initPresenter() {
        return new InquireFilterFragmentPresenter(getActivity());
    }
